...
exercises their agency,
showing the percentages of U.S.\ residential energy consumption 
for a variety of end uses~\citep[Fig.~3]{EIA_2018}.
...
can be aggregated to find three major categories 
...
Figure~\ref{fig:residential_elect_vs_home_size} shows the general trend of 
electricity consumption with respect to home size.
Figure~\ref{fig:residential_elect_vs_home_size} confirms intuition that
dwelling size matters for energy consumption. 
...
(See Chapter~\ref{chap:transportation}.)
...
furnace, A/C, insulation, windows
