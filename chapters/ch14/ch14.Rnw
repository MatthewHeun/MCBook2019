...
you have been confronted with the knowledge that the planet is not in good shape.
You may be overwhealmed by the sustainabilty challenges that confront us.
It is difficult, if not impossible, 
to be both fully aware of the state of the planet and know precisely what to do.
If that conundrum leads to lament, that's OK.
Lament occupies the space between the way we want things to be and they way they are
when we don't know how to make things right.

**** Reference my Beyond Stewardship chapter here. ****

But working through the lament 
At its best, lament can lead to meaningful action and recommitment to do better.

%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
\begin{mcframe}[0.90\textwidth](0.85\textwidth)
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``Live like the crisis is urgent. 
...
To that end, this chapter collects and summarizes 
steps to be taken by individuals to improve the state of the planet.
...
\begin{table}
\caption[Personal Q\&A]{Personal Q\&A}
\centering
\begin{tabular}{@{}ll@{}}
\toprule
