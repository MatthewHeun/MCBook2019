...
\chapter{Systems Thinking (MKH 4)}
...
Many of those interactions are highlighted in previous chapters, and the entities include
the environment (air, water, plants, animals, food, agriculture),
...
The purpose of this chapter is to introduce systems thinking\index{systems thinking}
...
Example systems are the Earth, the solar system, a volleyball team, an ant, a bacterium.
...
in ways that allow elements to interact to each other within the system.
...
Items 4a, 4b, and 4c are contained by element 3, but
they have sibling relationships among themselves.
Two systems with identical holons will be have differently 
...
A volleyball team that won the first set of a match handily goes into a funk, 
losing the first 8 points of the second set.
...
the chaotic behavior of systems described here is that 
...
can produce wildly different system behavior.
But in the real world, 
the infinitesimally small perturbation is indistinguishable from zero to within finite measurement uncertainty.
Thus, a real-world system with apparently the same initial conditions 
(to within measurement uncertainty) 
can exhibit wildly different behavior in the real world,
as systems thinking predicts.
\end{mcframe}
...
A volleyball coach can position players to thwart a hitter who sends spikes ``down the line,''
but her team's defense will now be vulnerable to the opposing setter's ``dinks''
to the middle of her defense.
...
Former Michigan U.S.\ 3$^\mathrm{rd}$ district congressman Vernon Ehlers often said
``You can never do only one thing.''

This apparent system unpredictability extends to human actions intended to influence the behavior of systems.
A volleyball coach can \emph{think} she is motivating a player with a stinging critique 
but produce the opposite effect, player demoralization.
Human efforts to ``manage'' forests have decimated old growth trees.
Colloquially, we call this unpredictability \emph{unintended consequences}.\index{unintended consequences}
...
The phrase ``reversion to the mean'' is often employed 
to describe the self-correcting nature of systems.
...
due to the self-correcting behavior of the system.
The phrase ``rearranging the deck chairs on the Titanic'' describes how
some human activities are meaningless in the face of 
larger system behaviors.
...
\begin{itemize}
  ...
  \item Small changes a lower levels can create big changes in higher levels of a holarchy

  \item Specialization possible at lower levels of holarchy
  
  \item Large, technical projects (airlines, spacecraft, chemical plants) are possible (Directed Technological Holarchy, Graedel p. 217)
  
  \item Resistant to change (preserve structure).
...
\begin{itemize}
  ...
  \item Emergent behavior brings unintended consequences

  \item Difficult to understand, because many holons need to be analyzed (complicated)
  
  \item Impossible to understand high-level behavior by analyzing low-level holons only (complex)
  
  \item Collaborative technological holarchies are difficult to manage (Graedel p. 217)
  
  \item We have linear minds in a non-linear world (Meadows p. 91)
...
``Control volumes'' and ``free body diagrams'' serve to
break down systems to analyze their elements piece-by-piece, per holon.
Positional and electrical constraints expressed as equations 
put the systems ``back together,''
by describing the connections among the components/elements/holons.

Engineers must address the complexities of engineered systems
to produce engineered obejects that meet performance ``requirements'' and
fulfill their intended purposes, their teloi.
To do so, ``safety factors'' and mass, power, volume, and cost margins are applied.
Progress updates are delivered during ``design reviews'' reviews to master engineers who 
...
Complicated: Difficult (but not impossible) to understand system by analyzing lower-level holons

Complex: Can't understand system by taking it apart

Systems can resist change (be resilient in the face of external perturbation)

Everything is connected all the time

Effects cannot be tracked deterministically

Knowledge has limits (always!)

Knowledge is provisional, contextual, and contingent (but not arbitrary)

Intuition can be as important as rationality when dealing with complex systems

One way to intervene in an established (legacy) system is to question the boundaries! Deconstruct (shift) the framework.


Modeling is an attempt to understand/describe a system (via simplification, a reduction of complexity)

Necessarily, all modeling involves drawing boundaries: Free-body-diagrams and control volumes

Agents (in Agent-based modeling)

All models 

Amplify some aspects or behaviors of a system 

Dampen others

Descriptions of the world (models and theories) involve choice. 

Thus, there are normative aspects to dealing with complexity (what ``ought'' to be done when constructing a model)
...
\label{sec:example_systems}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Two chapters already discussed systems:

Energy-economy

Transport system

Questions to ponder about the transport system:

What problems was the existing transportation system designed to solve?

What problems do we have today?

Is our legacy transportation system resilient (resistant to change)? Why or why not?

Does the existing transportation system exacerbate or help to solve today’s problems?

What should we do with ``legacy'' systems?

Does the tendency to preserve structure in legacy systems 
mean that our existing transport system sees sustainable transportation as a ``virus'' that needs to be eliminated?
