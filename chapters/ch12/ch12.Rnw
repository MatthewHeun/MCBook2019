...
Many of the most difficult problems involve sustainability in one way or another. 
...
end-use of energy-consuming machines
...
\end{enumerate*}
%
STCT indicates that benefits would be found by widening the system boundary
to include 
worldwide energy supply chains\index{supply chains!energy} and
the worldwide economy to obtain
...
leaves consumers with more money in their wallets.\index{energy efficiency!savings from}
...
that includes the economy (e.g., the CEE system)
...
A system with the expanded boundary could be called
the climate-energy-economy-policy (CEEP) system.

To those with an STCT mindset, 
developing and implementing policies are fraught.
Limits to human knowledge mean that no policy is perfect, and 
a large dose of humility is required for policymakers.
In fact, the limits to human knowledge are highlighted by
the fact that knowledge is contingent upon boundary placement.
...
Efficiency improvements can be legislated via efficiency \emph{standards}
or by economic incentives such as \emph{tax breaks} or \emph{rebates}  
for purchasing high-efficiency machines.
To ``change the connections,''
policymakers might mandate that sellers  
must provide information to buyers about the embodied CO$_2$ content 
of product choices.
