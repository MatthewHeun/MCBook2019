...
The connections between enclosed (lower-level) and enclosing (higher-level) holons in a holarchy
allow specialization of function. 
Higher levels of a holarchy often serve to organize and utilize the outputs from elements in lower levels of the holarchy.
Such organization enables metabolisms, organisms, and manufacturing supply chains.
