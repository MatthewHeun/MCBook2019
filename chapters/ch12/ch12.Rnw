...
System challenges arise from emergent behaviors.
...
Large, technical projects, such as airlines, spacecraft, chemical plants, are possible in what 
Graedel and Allenby call ``directed technological holarchies''~\cite[p.~217]{Graedel:2010ab}.

What if one desires to change the way a system functions? 
If one views, for example, Earth's climate, worldwide energy supply chains, and the worldwide economy
as a system, what options exist for changing the system to emit less CO$_2$ and 


If we want to intervene in a system to change its functioning and 
...
small changes to elements in lower levels can create big changes in higher levels of a system.


Resistant to change (preserve structure).
