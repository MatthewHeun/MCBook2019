...
System thinking and complexity theory (STCT) are valuable
...
Furthermore, 
STCT indicates that the boundary\index{system boundaries}
around the CEE system will amplify some aspects of real-world behavior and
dampen others.
...
Indeed, the rebound effect and backfire discussed in Section~\ref{sec:energy_transitions}
...
A narrow system boundar around end-use machines closes off the possibility 
to observe the rebound effect or backfire.
A STCT mindsent and an expanded system boundary
that includes the economy (e.g., the CEE system)
enable consideration of the rebound effect and backfire.

To those with a STCT mindset, 
...
the correctness of boundary placement can be evaluated for each system analysis.
For the CO$_2$ emissions example,
redrawing the CEE system boundary to 
