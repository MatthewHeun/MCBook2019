...
If one views, for example, Earth's climate,
worldwide energy supply chains\index{supply chains!energy}, and
the worldwide economy
as a system, what options exist for changing that system to emit less CO$_2$?\index{emissions!CO$_2$}
Meadows~\cite[Ch.~6]{Meadows:2008aa} gives an extensive list of 
``leverage points''\index{leverage points!of a system}
...
First, systems are difficult to understand, because 
many holons are present and need to be analyzed.
(Systems are complicated.\index{systems!complicated})
Second, even if it were possible to understand the behavior of \emph{all} holons, 
...
(Systems are complex.\index{systems!complex})
Third, and even worse, the unpredictability of high-level system behavior
...
Fourth, all of the above can make systems, 
including collaborative technological holarchies\index{directed technological holarchies},
difficult to manage.
Finally, the self-preservation characteristic of systems means that 
they have a ``mind of their own'' and can seemingly resist attempts to change them.
Maddeningly, for the volleyball coach,
sometimes substitutions have little effect or make team performance worse
due to some unexpected interactions between the holons (players).
...
The study of systems and their complex interactions
is called \emph{complexity theory}\index{complexity theory}~\cite{Cilliers:1998aa}. 
In the previous section, 
systems were described as being both complicated and complex.
Systems are complicated, because they are difficult
(but not impossible) to understand system by analyzing lower-level holons.
But systems, especially large systems with many connections,
are also complex, meaning that they \emph{can't} be understood by taking them apart. 
For example, a spacecraft\index{spacecraft} is complicated, because it has many parts.
It \emph{might} be possible to learn some aspects of spacecraft behavior
simply by studying its parts.
But it would be impossible to predict the social, psychological, and political
effects of Sputnik\index{Sputnik} by studying its holons.
A spacecraft is complex.

Many tenents of complexity theory have been discussed in preceding sections:
%
\begin{enumerate*}[label={(\alph*)}]
	
  \item systems can't understood by taking them apart,
  
  \item systems are resilient to external perturbation,
  
  \item all modeling is an attempt to understand or describe a system 
        via simplification, a reduction of complexity, and 
  
  \item necessarily, all modeling involves drawing boundaries
        (as in free-body-diagrams and control volumes).
  
\end{enumerate*}

But further insights emerge from complexity theory.

In a system, everything is connected all the time.
Attempts to isolate parts of a system are futile.
Emergent behavior means that 
effects cannot be tracked or predicted deterministically.
Thus, and importantly, human knowledge has limits, always!
Human knowledge is not arbitrary, but it is 
provisional, contextual, and contingent.

As a result, 
human intuition can be as important 
as scientific rationality when dealing with complex systems.
