...
Complexity theory takes system thinking in a philosophical direction
...
amplify some aspects or behaviors of a system and 
dampen other aspects or behaviors of a system. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{System thinking for sustainability}
\label{sec:system_thinking_for_sustainability}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Systems thinking and complexity theory are valuable
...
of reducing CO$_2$ emissions worldwide.

Systems thinking indicates that focusing narrowly on, say,
the efficiency of end-use of energy-consuming machines
(such as cars, lights, and furnaces)
will be insufficient to address the challenge, because 
everything is interconnected, always.
Focusing on end-use machines would neglect
%
\begin{enumerate*}[label={(\alph*)}]
	
  \item economic imperatives that lead to energy consumption and
  
  \item the energy conversion chain that supplies energy to end users.
    
\end{enumerate*}
%
System thinking and complexity theory would suggest widening the system boundary
to include the economy and the energy conversion chain.
And they caution against thinking that a system comprised of
Earth's climate,
worldwide energy supply chains\index{supply chains!energy}, and
the worldwide economy
will be sufficient to find a solution to the CO$_2$ emissions challenge.





(Note that defining system boundaries around these three entities is \emph{already} 
a potentially-problematic move, because
the system is a simplified version of reality that excludes other important factors, 
such as government, education, etc.)





Furthermore, assume that it is desired to change the system to significantly reduce CO$_2$ emissions.
The following insights emerge from system thinking and complexity theory.





, what options exist for changing that system to emit less CO$_2$?\index{emissions!CO$_2$}
...
\item Consider the existing transportation system. 
      What problems was it designed to solve?
      What problems do we have today?
      Is the legacy transportation system resilient (resistant to change)? Why or why not?
