...
With full knowledge of unintended consequences, 
the exhortations to 
``change the elements'' and ``change the connections''
...
Special accounts called \emph{green revolving funds}\index{green revolving funds}\url{http://greenbillion.org}
require that monetary savings from energy efficiency
be used for additional energy efficiency interventions,
thereby ensureing that savings don't ``leak out'' to the rest of the economy.
