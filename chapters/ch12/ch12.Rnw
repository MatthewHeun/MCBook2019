...
Many tenents of complexity theory emerge from systems thinking and
...
Thus, attempts to isolate parts of a real system 
...
how models and boundary placement mediate our knowledge 
...
i.e.\ human knowledge is always provisional, contextual, and contingent.
...
the rebound effect and backfire are behaviors that emerge from the CEE system
...
to observe an economy-wide rebound effect or backfire.
An STCT mindsent and an expanded system boundary
...
to encompass all factors related to CO$_2$ emissions, because
...
and, by virture of being exogenous to the CEE system, 
...
namely policies at the international level.%
\footnote{
...
endogenize the political system could unlock
...
After endogenizing politics within the CEEP system, 
an STCT mindset would indicate that
developing and implementing policies would be fraught
with potential problems.
Limits to human knowledge mean that no policy is perfect.
...
STCT predicts that entrenched interests will resist changes to the existing CEEP system.
Reasons for resisting change include financial (maintain existing reveunes and profits)
and social (maintain employment).
It becomes clear that a large dose of humility is required for policymakers.
STCT leads us to believe that all policies should be considered experiments
that are open to continued revision as more data becomes available.
...
Here, again, increasing adoption of renewable energy technologies might 
...
making the application processes for household rooftop solar arduous and complicated.
