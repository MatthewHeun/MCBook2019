...
read.table(text = data_string, sep="\t", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>% 
  tidyr::gather(key = "type", value = "value", "TOTAL U.S. tonne-kilometers of freight", "Air", "Truck", "Railroad", "Domestic water transportation", "Coastwise", "Lakewise", "Internal", "Intraport", "Pipeline") %>% 
  
  ggplot(mapping = aes(x = Year, y = "Tonne-kilometers of freight by transportation mode", linetype = type))+
  geom_line()+
  labs(x=element_blank(), y = expression("Tonne-kilometers of freight by transportation mode"))+
