...
\caption[Emissions by transport mode.]{CO$_2$ emissions by transportation mode. **** Need referece from Larisa. Redraw graph from data? Also, is this for the U.S. or world? ****}
...
Furthermore, figures~\ref{fig:personal_transport_by_mode} and~\ref{fig:freight_by_mode}
...
\caption[U.S.\ fuel economy.]{U.S.\ Fuel economy by year and vehicle type. **** Need reference. Need data. Need to redraw graph. ****}
\label{fig:US_mpg}
\end{figure}
...
**** Add a graph of biofuel consumption vs. time. ****
...
\item Research the term \emph{problem shifting}. 
      Identify other examples of problem shifting in which 
      a ``solution'' to one sustainability problem
