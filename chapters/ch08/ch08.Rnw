...
World's largest electric vehcile is a dump truch that never needs recharging.
...
1.7	Motorcycle/bike/other"
  read.table(text = data_string, sep="\t", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>% 
  dplyr::mutate(
    Modes = factor(Modes = c("Drive alone", "Carpool", "Public transportation", "Work from home", "Walk", "Motorcycle/bike/other"))) %>%
    ggplot()+
