...
\chapter{Land Use and Urban Planning (JVA 4)}
\label{chap:land_use_and_urban_planning}
...
This is approximately true. There are ways of creating more land. The Dutch system of building dykes to reclaim land from the ocean is one example. Planetary colonization could be another. Structures such as multi-story parking garages, high-rise apartment buildings, and off-shore oil platforms are also ways to multiply the {\i effective use} of the land. 
% Change in efficiency. but land is constant. 
Techniques that increase agricultural productivity {\i per acre} are effectively equivalent to creating more farm land. But, all these aside, it remains true that the surface area of planet Earth is a fixed quantity and that the various methods described above have very little relative effect on the total land available for human use.
