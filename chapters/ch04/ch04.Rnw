...
and that framework is applied here.
...
In the sections that follow, resource depletion and emissions are discussed in turn, 
with a focus on fossil fuel resource depletion and greenhouse gas (GHG) emissions.
...
Depletion of nonrenewable natural resources is a complex process
that involves natural processes, human wants and needs, and technology.
To describe the processes succinctly, we use a model from calculus.
All natural resources have a generation rate~($\dot{g}$)\index{resources!generation rate}, 
the speed with which the resource is re-created by natural or human processes.
...
The initial stock of a resource~($s$) is given by~$s_0$.
The stock level, the generation rate, and the consumption rate can be a function of time 
($s(t)$, $\dot{g}(t)$, $\dot{c}(t)$)
...
When $\dot{g} < \dot{c}$, a resource is depleting.
...
such as biomass.
For potentially renewable resources, 
...
For nonrenewable energy resources, such as coal, oil, and natural gas, 
$\dot{g} \ll \dot{c}$, and 
they are always being depleted by consumption.
...
All other things being equal, as the stock of a nonrenewable resource declines
extraction costs are expected to rise 
as increasingly-difficult sites are utilized,
such as oil from offshore platforms or kilometers-deep subsurface coal mines.
But all things are never equal, and 
technological advances put downward pressure on extraction costs, 
even as a resource depletes.  
In the end, extraction cost of an energy resource is a race between depletion and technology.

If society continues to consume a nonrenewable energy resource 
at rate $\dot{c}$ (which is $\gg \dot{g}$) indefinately,
its stock ($s(t)$) will eventually go to $0$.
But $s \rightarrow 0$ is not the expected long-run outcome for nonrenewable resources.
Rather, extraction (and $\dot{c}$, also) will cease when the cost of extraction exceeds the 
market price for the nonrenewable energy resource.
The fraction~($r$) of initial resource stock~($s_0$) that is ultimately recoverable 
is determined by the market price relative to the extraction costs.

If sustainabilty is defined as continuing an activity indefinitely
(see Chapter~\ref{chap:introduction}), 
$\dot{c} > 0$ is unsustainable for any nonrenewable resource with $\dot{c} \gg \dot{g}$.
I.e., any consumption of nonrenewable resources is, by definition, unsustainable.
Thus, ongoing consumption of coal, oil, and natural gas is unsustainable.
But given that modern society is thoroughly dependent upon nonrenewable energy sources,
a reasonable question arises: how long can we continue our current consumption trajectories
of non-sustainable energy resources such as coal, oil, and natural gas.
...
simple calculations can be performed to obtain estimates 
...
\footnote{
The estimation approach assumes that nonrenewable resource production
...
leaving $384 \; \mathrm{Gt}$ remaining to be extracted.
...
\end{equation}
%
Thus, this simple calculation indicates that the world will ``run out'' of coal in 2070.
The dynamics of nonrenewable energy resource depletion
will have more nuance and complexity than this simple calculation indicates. 
But a more-detailed analysis~\citep[p.~22]{Rutledge:2018aa}
puts the date at 2066, 
showing that a simple approach can still yield helpful insights.

Even these simple calculations can provide a timeframe for planning a transition 
away from nonrenewable resources before they are depleted.
...
before reaching the dangerous 1.5 \degC{} warming level caused by GHG emissions.
...
The time depletion timeframe (51 years) and the emissions timeframes (10--20 years) can be compared.
In this case, the emissions timeframe is more challenging than the depletion timeframe.
...
However, increasing energy efficiency is correlated with increasing GDP, and  
increasing GDP is correlated with increasing primary energy consumption~\citep{Heun:2019aa}.
...
\item Choose another nonrenewable resource besides coal. 
      ...
      What will be required for such a transition?
...
\item Find several critiques of the WWS proposal. 
      ...
      Given the criqiques and the additional costs, 
      what is an updated estimate for the cost of the WWS fossil-to-renewable transition?
      On annual basis, what percentage of world GDP does the updated cost of a WWS transition represent?
