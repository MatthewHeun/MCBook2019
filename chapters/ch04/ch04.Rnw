...
Depletion of non-renewable natural resources is a complex process
...
For non-renewable energy resources, such as coal, oil, and natural gas, 
$\dot{g} \ll \dot{c}$, and 
...
All other things being equal, as the stock of a non-renewable resource declines
...
If society continues to consume a non-renewable energy resource 
at rate $\dot{c}$ (which is $\gg \dot{g}$) indefinately,
its stock ($s(t)$) will eventually go to $0$.
But $s \rightarrow 0$ is not the expected long-run outcome for non-renewable resources.
Rather, extraction (and $\dot{c}$, also) will cease when the cost of extraction exceeds the 
market price for the non-renewable energy resource.
...
$\dot{c} > 0$ is unsustainable for any non-renewable resource with $\dot{c} \gg \dot{g}$.
I.e., any consumption of non-renewable resources is, by definition, unsustainable.
Thus, ongoing consumption of coal, oil, and natural gas is unsustainable.
But given that modern society is thoroughly dependent upon non-renewable energy sources,
...
But simple calculations can be performed to obtain estimates 
...
\footnote{
The estimation approach assumes that non-renewable resource production
...
Dividing the remaining coal by today's extraction rate gives the years of coal extraction remaining ($\Delta t$)
%
