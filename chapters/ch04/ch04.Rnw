...
Natural resources have generation rates and consumption rates.
The generation rate~($\dot{g}$) 
describes the speed with which the resource is re-created by natural or human processes.
The consumption rate~($\dot{c}$) 
describes the speed with which the resource is removed from nature.
The initial stock of the resource~($s$) is given by~$s_0$.
...
such as solar and some geothermal sites,
$\dot{g} \gg \dot{c}$, and the resource is virtually limitless.
For other renewable energy resources, 
such as biomass,
the relationship between $\dot{g}$ and $\dot{c}$ depends on local growing conditions and harvesting.
