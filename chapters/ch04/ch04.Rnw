...
and that framework is applied in this chapter.
...
when increasingly-difficult sites are utilized.
Examples include oil from offshore platforms or kilometers-deep subsurface coal mines.
...
at rate $\dot{c}$ indefinately (remember that $\dot{c} \gg \dot{g}$ for a nonrenewable resource),
...
leaving $384 \; \mathrm{Gt}$ in the ground.
...
\end{equation}
%
This simple calculation indicates that the world will ``run out'' of coal in 2070.

As discussed above, 
the dynamics of nonrenewable energy resource depletion
exhibit more nuance and complexity than this simple calculation entails.
In fact, extraction rates are expected to decline as nonrenewable resources
are depleted.
Despite its obvious shortcoming, 
the result from this simple method nearly matches
a more-detailed analysis by~\citet[p.~22]{Rutledge:2018aa}, which 
puts the ``end of coal'' date at 2066.
Indeed, simple methods can sometimes yield helpful insights.
In this case, simple methods can provide a timeframe for planning a transition 
...
Figure~\ref{fig:LLNL_sankey} provides data to calculate the energy efficiency\index{energy efficiency}
of each sector,
...
(wind, water, and solar, or WWS)\index{wind, water, and solar}\index{WWS|see wind, water, and solar}
