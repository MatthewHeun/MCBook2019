...
library(extrafont)   # Allows import of LibreCaslon font
library(ggplot2)     # For graphing functions
library(magrittr)    # For pipe operator, %>% 
library(tinytex)     # Gives a tiny LaTeX distribution in R.
options(tinytex.verbose = TRUE)
library(VennDiagram) # For the 3-circle sustainability diagram.

# Set up LibreCaslon font. invisible() swallows the text output.
font_import(paths = "Libre-Caslon-Text-master", prompt = FALSE)
loadfonts()
@
